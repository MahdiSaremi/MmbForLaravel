...
namespace Mmb\Laravel\Support\AttributeLoader;
...
trait HasAttributeLoader
{
    ...
    public static function getNormalizedCallingMethod(string $method, array $args, Closure $callback = null)
    {
        ...
        foreach($parameters as $parameter)
        {
            ...
            if($findBy = static::getParameterAttributeOf($method, $parameter->getName(), FindBy::class))
            {
                if($value instanceof Model)
                {
                    ModelFinder::store($value);
                    $value = $findBy->key === '' ? $value->getKey() : $value->getAttribute($findBy->key);
                }
            }
